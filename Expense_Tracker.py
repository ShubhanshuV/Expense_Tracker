...
class Expense:
    #Initialisation of tables and SQL queries to extract/add data
    def __init__(self):
...
class ExpenseTracker:
    ...
    def create_menu(self):
        #Title bar Menu creation
       
        menubar = tk.Menu(self.root)

        
        expense_menu = tk.Menu(menubar, tearoff=0)
        # expense_menu.add_command(label="Add User", command=self.create_user)
        expense_menu.add_command(label="View Expense List", command=self.view_expense)
        expense_menu.add_command(
            label="Pie Chart by Category", command=self.open_expense_viewer
        )
        expense_menu.add_command(
            label="Monthly Expense Comparision",
            command=self.draw_monthly_expense_graph,
        )
        expense_menu.add_command(label="Export Expenses", command=self.export_expenses)
        menubar.add_cascade(label="Tools ", menu=expense_menu)

        self.root.config(menu=menubar)

    def on_date_select(self, event):
        #Fudure date selection handling
    ...
    def show_monthly(self):
        ...
        if user is None or user == "" or user == "--Select User--":
            ...
            return
        data = self.db.view_expenses_month(user)
        print(data)
        canvas = tk.Canvas(self.root, width=500, height=500)
        canvas.grid(row=6, column=0, columnspan=4)
        ExpenseTracker.create_bar_graph(canvas, [x[1] for x in data])
    ...
    def draw_monthly_expense_graph(self):
        ...
        def generate_graph():
            ...
            if year != "--Select Year--":
                ...
                fig, ax = plt.subplots()
                ax.bar(all_months, amounts,color="#141F82")
    ...
    def export_expenses(self):
        #exporting expense to a csv file
